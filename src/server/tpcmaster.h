...
typedef struct tpcslave {
  ...
  pthread_rwlock_t tpc_slave_lock; /* Lock to prevent GETs from accessing slaves 
                                      that are undergoing tpc PUTs/DELs, and multiple simultaneous PUTs/DELs. */
} tpcslave_t;
...
typedef struct tpcmaster {
  pthread_rwlock_t tpc_lock;    /* Lock to ensure only one tpc request is handled at once. */
  ...
  pthread_rwlock_t slave_lock;  /* A lock used to protect the list of slaves, and the slave_count. */
...
void tpcmaster_handle_tpc_error(tpcmaster_t *master, kvmessage_t *respmsg, char *error_msg);
