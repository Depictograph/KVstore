...
//  server->current_threads = 0; 
//  pthread_mutex_init(&server->thread_lock, 0);
  ...
  /* create max_threads threads to loop doing handle */
  int tnum;
  pthread_t worker_threads[server->max_threads];
  for(tnum=0; tnum < server->max_threads; tnum++) {
//    pthread_t new_thread;
    pthread_create(&worker_threads[tnum], NULL, handle, (void*) server);
  }
  ...
  //Done listening, push NOOPs to the queue to wake up the remaining threads so they can exit
  for (tnum=0; tnum < server->max_threads; tnum++) {
    wq_push(&server->wq, (void *) NOOP);
  }
  /* Wait for all the threads to finish */
  for (tnum=0; tnum < server->max_threads; tnum++) {
    pthread_join(worker_threads[tnum], NULL);
  }
  /* Remove the rest of the NOOPs from the WQ */
  wq_clear(&server->wq);
