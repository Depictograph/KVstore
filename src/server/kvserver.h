...
typedef struct kvserver {
  ...
  kvmessage_t* pending_request; /* A kvmessage that is stored in between PUT/DEL and COMMIT/ABORT messages from the master. */
  pthread_mutex_t pending_request_lock;  /* lock on pending_request. Required for the condition that 2 PUT/DEL requests can be received before a commit or abort */
} kvserver_t;
