...
int kvcacheset_get_nonexistant(void) {
  char *retval = NULL;
  int ret;
  ret = kvcacheset_get(&testset, "mykey1", &retval);
  ASSERT_EQUAL(retval, NULL);
  ASSERT_EQUAL(ret, ERRNOKEY);
  return 1;
}

int kvcacheset_del_nonexistant(void) {
  int ret;
  ret = kvcacheset_del(&testset, "mykey1");
  ASSERT_EQUAL(ret, ERRNOKEY);
  return 1;
...
test_info_t kvcacheset_tests[] = {
  ...
  {"GET on a key that is not in the cache", kvcacheset_get_nonexistant},
  {"DEL on a key that is not in the cache", kvcacheset_del_nonexistant},
  NULL_TEST_INFO
};
