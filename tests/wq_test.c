...
/* Unit test for wq_is_empty. */
int wq_is_empty_unit_test(void) {
  int ret, item;
  ret = wq_is_empty(&testwq);
  ASSERT_EQUAL(ret, 1);
  wq_push(&testwq, (void *) 162);
  ret = wq_is_empty(&testwq);
  ASSERT_EQUAL(ret, 0);
  item = (intptr_t) wq_pop(&testwq);
  ASSERT_EQUAL(item, 162);
  ret = wq_is_empty(&testwq);
  ASSERT_EQUAL(ret, 1);
  return 1;
}

void *wq_pop_test_thread_partial(void* aux) {
  int item;
...
int wq_wait_partial_test(void) {
  pthread_t pop_threads[20];
  int ret; 

  for (int i = 0; i < 10; i++)
    pthread_create(&pop_threads[i], NULL, wq_pop_test_thread_partial, NULL);
  for (int i = 0; i < 20; i++)
    wq_push(&testwq, (void *) (intptr_t) i);
  ...
  wq_push(&testwq, (void *) (intptr_t) 0);
  sleep(1); /* give some time for any threads to pop the new element */
  ret = wq_is_empty(&testwq);
  ASSERT_EQUAL(ret, 0);
  return 1;
}

int wq_clear_test(void) {
  int ret;
  for (int i = 0; i < 20; i++)
    wq_push(&testwq, (void *) (intptr_t) i);
  ret = wq_is_empty(&testwq);
...
test_info_t wq_tests[] = {
  ...
  {"Tests that is_empty returns true if and only if the WQ is empty", wq_is_empty_unit_test},
  {"Tests that exactly the number of pushed items will be popped.", wq_wait_partial_test},
  {"Tests wq_clear correctly empties the WQ", wq_clear_test},
  NULL_TEST_INFO
};
