...
int socket_server_start_stop_test(void) {
  int ret = 0;
  int i;
  for (i = 0; i < 10; i++) {
    ret += socket_server_multiple_test();
    // Wait to make sure the server_thread actually finished. 
...
test_info_t socket_server_tests[] = {
  ...
  {"Tests that starting and stopping the server repeatedly works", socket_server_start_stop_test},
  NULL_TEST_INFO
};
