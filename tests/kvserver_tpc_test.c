...
int kvserver_tpc_del_abort(void) {
  //setup with PUT
  reqmsg.type = PUTREQ;
  reqmsg.key = "blade";
  reqmsg.value = "Earth Shock on a Twilight Drake";
  kvserver_handle_tpc(&testserver, &reqmsg, &respmsg);
  reqmsg.type = COMMIT;
  reqmsg.key = NULL;
  reqmsg.value = NULL;
  ...
  kvserver_handle_tpc(&testserver, &reqmsg, &respmsg);
  ASSERT_EQUAL(respmsg.type, GETRESP);
  ASSERT_STRING_EQUAL(respmsg.value, "Earth Shock on a Twilight Drake");
  return 1;
}
...
test_info_t kvserver_tpc_tests[] = {
  ...
  {"Valid DEL request followed by an ABORT", kvserver_tpc_del_abort},
  NULL_TEST_INFO
};
